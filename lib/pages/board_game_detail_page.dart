...
class BoardGameDetailPage extends StatelessWidget {
  const BoardGameDetailPage({super.key, required this.venue});
    ...
    body: ListView(
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              const Icon(Icons.casino_outlined, color: Color(0xFF2563EB), size: 29),
              const SizedBox(width: 12),
              Expanded(
                child: Text(
                  venue.summary,
                  style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, height: 1.45, color: AppColors.textPrimary),
                ),
              ),
            ],
          ),
        ...
        OutlinedButton.icon(
          onPressed: () => openOfficialWebsite(
            ...
            nativePageBuilder: (_) => _BoardGameWebViewPage(title: venue.name, url: venue.officialUrl),
          ),
  ...
  Future<void> _pickDateAndSchedule(BuildContext context) async {
    ...
    await context.read<AppState>().addBoardGameToSchedule(venue, at);
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(
      ...
      children: [
        ...
        SizedBox(width: 70, child: Text(label, style: const TextStyle(color: AppColors.textSecondary))),
...
class _BoardGameWebViewPage extends StatefulWidget {
  const _BoardGameWebViewPage({required this.title, required this.url});
  final String title;
  final String url;

  @override
  State<_BoardGameWebViewPage> createState() => _BoardGameWebViewPageState();
}

class _BoardGameWebViewPageState extends State<_BoardGameWebViewPage> {
